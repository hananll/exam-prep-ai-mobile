...
export default function QuestionScreen({ navigation, route }: Props) {
  ...
  const fetchSorular = async () => {
    try {
      ...
      const soruListesi: SoruApi[] = Array.isArray(data) ? data : (data.data || []);
      
      if (response.ok && soruListesi.length > 0) {
        setSorular(soruListesi);
        setCurrentIndex(0);
        setSelectedOption(null);
      } else {
        setSorular([]);
        Alert.alert('Bilgi', 'Bu teste henüz soru eklenmemiş veya sorular silinmiş.');
      }
    } catch (error) {
  ...
  if (sorular.length === 0) {
    return (
      <SafeAreaView style={[styles.safeArea, { justifyContent: 'center', alignItems: 'center', padding: 20 }]}>
        ...
        <TouchableOpacity
          style={[styles.nextButton, { marginTop: 20, paddingHorizontal: 20 }]}
  ...
  const handleNext = () => {
    if (currentIndex < sorular.length - 1) {
      ...
      sorular.forEach((soru) => {
        const secilen = allAnswers[soru.id];
        const konu = soru.konu_adi || lessonName;

        if (!konuMap[konu]) {
          konuMap[konu] = { dogru: 0, toplam: 0 };
        }
      ...
      const gecenSure = `${mins} Dk ${secs} Sn`;
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        {isAnswered && !isCorrect && currentQ.cozum_aciklamasi ? (
